...
router.get("/", async (req, res) => {
  try {
    ...
    if (genre) filter.genres = genre; // Can pass genre ObjectId
...
// GET media by genre NAME (NEW - This is what your frontend needs!)
router.get("/genre/:genreName", async (req, res) => {
  try {
    const genreName = req.params.genreName.toLowerCase();

    // Find the genre by name first
    ...
    // Find all media with this genre
    const media = await Media.find({ genres: genre._id })
      .populate("genres", "name description")
      .sort({ rating: -1 })
      .limit(20);
    ...
    console.error(
      `Error fetching media for genre ${req.params.genreName}:`,
      error
    );
    res.status(500).json({
      success: false,
...
// GET media by genre ID (Keep this for backwards compatibility)
router.get("/genre/id/:genreId", async (req, res) => {
  try {
    const media = await Media.find({ genres: req.params.genreId })
      .populate("genres", "name description")
      .sort({ rating: -1 })
      .limit(20);
...
router.post("/add/media", async (req, res) => {
  try {
    ...
    await media.populate("genres", "name description");
